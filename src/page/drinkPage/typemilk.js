...
export default function Typemilk (){
...
$.ajax(
    {
      url: 'http://54.88.4.245/api/drink/milk_fact.php ',
      type: 'post',
      cache: false,
      async: false,
      dataType: 'json',
    ...
    for(var i=0;i<num;i++){
        var pic=itemDataMilk[i].img;
    }
    
    console.log("drinkid", drinkid);
    console.log(itemDataMilk[0]);

    return (
      <>
        
        <h1>{itemDataMilk[0].title}</h1>

        <input type='button' value='加入心願清單' className="hopebt"  />
        <div>
        <img className="drinkimg" src={itemDataMilk[0].img} />
        </div>
       ...
       <h4>原料:{type[0]} {type[1]} {type[2]} {type[3]} {type[4]} {type[5]} {type[6]} {type[7]}</h4>
        ...
        <tr>
            <td>{itemDataMilk[0].vol}</td>
            <td>{itemDataMilk[0].cal}cal</td>
            <td>${itemDataMilk[0].pri}</td>
        </tr>
        <tr>
            <td></td>
            <td>cal</td>
